...
function imgui.OnDrawFrame()
    if imgui_window.v.v then
        ...
        imgui.Begin('AutoReport Settings', imgui_window.v)
        ...
        if doesFileExist(path) then
            ...
            if f then
                ...
                if content and content:match("(%d+):") then
                    local expiration_time = content:match("(%d+):")
                    local time_left = formatTimeLeft(expiration_time)
                    imgui.TextColored(imgui.ImVec4(0, 1, 0, 1), u8'Статус подписки: Активна')
                    imgui.Text(u8'Осталось времени: ' .. u8(time_left))
                end
            end
        else
            imgui.TextColored(imgui.ImVec4(1, 0, 0, 1), u8'Статус подписки: Неактивна')
        end
        
        -- Добавляем отображение HWID
        local hwid = getSecureHWID()
        if hwid then
            imgui.Separator()
            imgui.TextColored(imgui.ImVec4(1, 1, 1, 1), u8'Ваш HWID:')
            imgui.TextColored(imgui.ImVec4(1, 0, 0, 1), hwid)
        end
...
function main()
    ...
    sampAddChatMessage('[AutoReport] {ffffff}Скрипт создан {ff0000}Papa_Neurowise {ffffff}[{00ccff}Администратор Yava{ffffff}]. Версия: {00cc00}2.0', 0x7ef542)
...
function sampev.onServerMessage(color, text)
    if active and _0x1.is_activated then
        -- Улучшенное регулярное выражение для более точного определения репортов
        if text:find('%[([^%]]+)%] от ([^%[]+)%[%d+%]:') then
            ...
            if current_time - _0x1.last_report_time >= MIN_REPORT_DELAY then
                ...
                lua_thread.create(function()
                    sendOtCommand()
                    wait(1)
                    sendOtCommand() -- Дополнительная отправка
                end)
